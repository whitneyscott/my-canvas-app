...
describe('consolidation regression guardrails', () => {
  it('uses discussion_topics for discussions and announcements paths', () => {
    const courseId = 42;
    expect(TEST_CONFIG.discussions.updatePath(courseId, 7)).toContain('/discussion_topics/');
    expect(TEST_CONFIG.announcements.updatePath(courseId, 7)).toContain('/discussion_topics/');
  });
  ...
  it('keeps announcements params aligned with announcement grid date fields', () => {
    ...
    expect(params).toEqual(
      expect.arrayContaining(['title', 'message', 'allow_rating', 'delayed_post_at', 'lock_at', 'published']),
    );
  ...
  it('keeps modules params aligned with runtime module fields', () => {
    ...
    expect(params).toEqual(expect.arrayContaining(['name', 'position', 'unlock_at', 'published']));
