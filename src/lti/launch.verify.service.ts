...
export class LaunchVerifyService {
  ...
  async verify(
    idToken: string,
  ): Promise<jose.JWTPayload & Record<string, unknown>> {
    ...
    const payload = jose.decodeJwt(idToken) as jose.JWTPayload &
    ...
    const jwk =
      jwks.keys?.find((k: { kid?: string }) => k.kid === unprotected.kid) ||
      jwks.keys?.[0];
    ...
    try {
      await jose.compactVerify(
        idToken,
        await jose.importJWK(jwk, unprotected.alg),
      );
    } catch (err) {
      if (
        String(err).includes('2048') ||
        String(err).includes('modulusLength')
      ) {
