...
**Where we are (Mar 2026):** **Scanner:** **22** violation fixtures + clean (**23** rows) on pages + assignments; **verified strict pass 23/0** (`report-qa-1774710091861.json`). **Dual-option** rows use `dual_option_choice`; **`QA_FIX_AUTO=1`** runs suggested preview/apply with `edited_suggestion` for those rows. Next gaps: optional non-Sprout iframe parity, **lang** scanner, **broken-link** E2E, **CI** wiring.
