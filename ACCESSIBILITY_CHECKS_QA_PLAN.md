...
4. ~~Dual-option fix validation for `aria_hidden_focusable` and `table_layout_heuristic` (§4.2.2).~~ — **partial** (runner + fixtures + `dual_option_choice`; not yet verified on OSS in plan text).
...
**Where we are (Mar 2026):** **Scanner:** **22** violation fixtures + clean (**23** rows) on pages + assignments, including **dual-option** rows with `dual_option_choice` for `aria_hidden_focusable` and `table_layout_heuristic`. **`QA_FIX_AUTO=1`** now runs **suggested** dual-option preview/apply using manifest `dual_option_choice`. Rebuild manifest and re-verify strict **pass 23/0** on OSS. Next gaps: optional non-Sprout iframe parity, **lang** scanner, **broken-link** E2E, **CI** wiring.
...
- [x] Fixture definitions (`test/fixtures/accessibility-qa/fixtures.json`) — pages + assignments **strict scanner** set (**22** violation rows + clean control), including **dual-option** `dual_option_choice` rows for `aria_hidden_focusable` and `table_layout_heuristic`, plus **SproutVideo `iframe_missing_title`**. **Still to add:** franc-length lang fixtures (when scanner emits), optional YouTube/Vimeo/Docs iframe parity rows, broken-link strategy, more content types.
...
- [x] Runner (`scripts/accessibility-qa-runner.js`) — manifest load + schema validation, **strict/best-effort** scanner assertions, JSON report, **`QA_FIX_AUTO`** for **`auto`** rules and for **`dual_option` + `dual_option_choice`** suggested fixes (`trySuggestedFixWithEditedChoice`). **Verified:** strict **19/0** before dual-option split (`report-qa-1774709610791.json`); **re-verify 23/0** after rebuild. **Still missing:** AI token assertions, broader suggested-flow simulation, regression baseline diff.
