...
**Implementation status (this repo):** Phase 1 builder (`accessibility-qa-builder.js`) seeds **pages**, **assignments**, **announcements**, **discussions**, and a **merged syllabus** row for scanner-supported types; **quizzes** / **modules** are still not wired. After `nest build`, the builder loads `ACCESSIBILITY_FIXABILITY_MAP` from **`dist/canvas/canvas.service.js`** and writes `fix_strategy`, `uses_ai`, `is_image_rule`, `uses_second_stage_ai`, `dual_option`, and `pending_heuristic` on manifest rows (`npm run qa:accessibility:build` runs `nest build` first). Phase 2 runner: **scanner assertions** for manifest fixtures; **strict tier** verified **green** at **68/68** on local Canvas OSS — **`test/fixtures/accessibility-qa/report-qa-1774715833910.json`** (2026-03-28) scan-only, including ann/disc clones per page violation + **`syllabus_composite_page_violations`**. **Fix-mode** (`npm run qa:accessibility:run:fix` after fresh `qa:accessibility:build`): **68/68** strict, **`fix_fail=0`**, **`test/fixtures/accessibility-qa/report-qa-1774717260033.json`** (2026-03-28). Earlier milestone **23/23**: `report-qa-1774710091861.json` (pages + assignments–only manifest). Scan matching uses **`resource_type` + `resource_id`** aligned with Canvas (manifest **`resource_id`** from API **`url` before `page_id`**; `getCoursePages` merges show-response `url` / `page_id` so scan keys match the manifest). Optional **`QA_FIX_AUTO=1`** path (preview → apply → re-scan for non-AI `auto` rules; skips `dual_option` and `uses_ai` unless `QA_FIX_AUTO_AI=1`). CLI Canvas API base: **`CANVAS_BASE_URL`** or **`CANVAS_QA_PROFILE`** (`docker` vs `online` defaults); scripts load `.env` without overriding the shell. Scanner emits **`text_justified`**, **`font_size_too_small`**, **`iframe_missing_title`**, and refined **`aria_hidden_focusable`** / **`table_layout_heuristic`** (see dual-option QA notes). **Still open:** strict AI token assertions, quizzes/modules, file uploads, CI job wiring (runner supports **`QA_BASELINE_REPORT`** vs a golden `report-qa-*.json`).
...
5. Extend to all content types in manifest; quiz capability probe skips documented in report. — **partial** (pages, assignments, announcements, discussions, syllabus in scan + builder; **quizzes** / **modules** still out of scope).
...
9. Regression baseline compare (strict-only optional for CI). — **partial** (`QA_BASELINE_REPORT` / `QA_BASELINE_PATH` in `accessibility-qa-runner.js`: per-fixture strict regression vs a prior `report-qa-*.json`; no CI job wired yet).
...
**Where we are (Mar 2026):** **Scanner** supports **`pages`**, **`assignments`**, **`announcements`**, **`discussions`**, **`syllabus`** (`ACCESSIBILITY_SUPPORTED_TYPES`). **Builder** mirrors every **page** violation into matching **announcement** + **discussion** topics (titles `[QA] Ann …` / `[QA] Disc …`) and builds one **merged syllabus** body from all page-violation HTML with manifest row **`syllabus_composite_page_violations`**. **Strict row count** scales with `fixtures.json` (base pages + assignments + clean + ann/disc clones + composite syllabus). **Verified:** strict **68**/**68** scan-only, **`test/fixtures/accessibility-qa/report-qa-1774715833910.json`** (2026-03-28). **Fix-mode verified:** strict **68**/**68**, **`fix_fail=0`**, **`test/fixtures/accessibility-qa/report-qa-1774717260033.json`** (2026-03-28; fresh build before run, RUNBOOK §2.1). Optional **`npm run qa:accessibility:run:fix:ai`** with **`ANTHROPIC_API_KEY`** for `uses_ai` auto rows. Next gaps: **`quizzes`** / **`modules`** (not in scan yet), optional non-Sprout iframe parity, **CI** wiring.
...
- [x] Builder (`scripts/accessibility-qa-builder.js`) — course create/reuse, **`--force-rebuild`** / **`npm run qa:accessibility:build:force`** (DELETE QA course when permitted), **pages + assignments + announcements + discussions** (discussion API), **merged syllabus** from page violations, manifest emit, **registry sync from `dist`** after `nest build`. **Still missing:** **quizzes/modules** (scan does not load them yet), file-upload pipeline.
- [x] Runner (`scripts/accessibility-qa-runner.js`) — manifest load + schema validation, **strict/best-effort** scanner assertions, JSON report, **`QA_FIX_AUTO`** for **`auto`** / dual-option suggested fixes, **`QA_LINK_SCAN_RETRIES`** refetch for **`link_broken`** rows, **`QA_BASELINE_REPORT`** strict regression vs prior report. **Still missing:** AI token assertions, broader suggested-flow simulation, full `link_broken` fix-preview teacher-URL simulation, CI wiring for baseline.
