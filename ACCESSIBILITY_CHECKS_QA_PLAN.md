...
**Implementation status (this repo):** Phase 1 is **partial**: `accessibility-qa-builder.js` implements **pages + assignments** only; skips other content types with `skipped_reason: content_type_not_implemented`. After `nest build`, the builder loads `ACCESSIBILITY_FIXABILITY_MAP` from **`dist/canvas/canvas.service.js`** and writes `fix_strategy`, `uses_ai`, `is_image_rule`, `uses_second_stage_ai`, `dual_option`, and `pending_heuristic` on manifest rows (`npm run qa:accessibility:build` runs `nest build` first). Phase 2 runner: **scanner assertions** for manifest fixtures; **strict tier** last verified **green on local Canvas OSS** with **17** strict rows (Mar 2026); after **SproutVideo iframe** fixtures rebuild, expect **19** strict rows (**18** violations + clean control). Scan matching uses **`resource_type` + `resource_id`** aligned with the API (wiki **`page` URL slug** as `resource_id`, not numeric id only). Optional **`QA_FIX_AUTO=1`** path (preview → apply → re-scan for non-AI `auto` rules; skips `dual_option` and `uses_ai` unless `QA_FIX_AUTO_AI=1`). CLI Canvas API base: **`CANVAS_BASE_URL`** or **`CANVAS_QA_PROFILE`** (`docker` vs `online` defaults); scripts load `.env` without overriding the shell. Scanner now emits **`text_justified`** and **`font_size_too_small`** (inline `style` pass). **Still open:** dual-option variant tests, strict AI token assertions, broader content types, file uploads.
...
2. ~~Scanner assertion engine; respect **strict** vs **best_effort** gates.~~ — **done** for **pages + assignments** in manifest (re-verify strict count after each fixture batch; **19** rows after SproutVideo iframe additions).
...
4. Dual-option fix validation for `aria_hidden_focusable` and `table_layout_heuristic` (§4.2.2). — **not done**
...
**Where we are (Mar 2026):** Local Canvas OSS + Nest QA headers can run **`qa:accessibility:build`** / **`qa:accessibility:run`** with **strict scanner assertions** for **18** pages+assignments **violation** fixtures + clean control (**19** manifest rows; rebuild after pull). **`iframe_missing_title`** scanner + SproutVideo-style fixtures + `suggestIframeTitleFromSrc` Sprout label shipped in repo. Next gaps: **Option A/B HTML variants** for dual-option *fix* QA, optional non-Sprout iframe parity fixtures, **lang** scanner gaps, **broken-link** E2E, **CI** wiring.
...
- [x] Fixture definitions (`test/fixtures/accessibility-qa/fixtures.json`) — pages + assignments **strict scanner** set (**18** violation rows + clean control), including `aria_hidden_focusable`, `table_layout_heuristic`, and **SproutVideo `iframe_missing_title`** on both surfaces. **Still to add:** dual-option **fix** variants (Option A/B HTML per rule), franc-length lang fixtures (when scanner emits), optional YouTube/Vimeo/Docs iframe parity rows, broken-link strategy, more content types.
...
- [x] Runner (`scripts/accessibility-qa-runner.js`) — **done for v1:** manifest load + schema validation, **strict/best-effort** scanner assertions, JSON report, optional **`QA_FIX_AUTO`** path. **Verified:** strict **pass 17/0** on Canvas OSS before iframe fixtures (`report-qa-1774709251949.json`); **re-run** after rebuild for **19** rows. **Still missing:** dual-option *fix* tests, AI token assertions, suggested-flow simulation, regression baseline diff.
