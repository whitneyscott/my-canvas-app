...
Re-running the builder updates existing pages/assignments in place (idempotent by title/code/slug). Use **`--force-rebuild`** / **`npm run qa:accessibility:build:force`** to delete the QA course when the token allows `DELETE /courses/:id`, then recreate it; otherwise delete the course in Canvas UI.
...
**Implementation status (this repo):** Phase 1 builder seeds **pages**, **assignments**, **announcements**, **discussions**, **quizzes**, **module items** (Page + Assignment links under `[QA][A11y] Fixture Module`), **merged syllabus**, optional **course file PDF upload** for `PLACEHOLDER_COURSE_PDF` fixtures, and manifest **`canvas_capability`** (`quizzes_rich_text`, `module_items_seeded`, `course_pdf_upload` when upload fails). Scanner **`ACCESSIBILITY_SUPPORTED_TYPES`** includes **`quizzes`** and **`modules`** (module items resolve Page/Assignment HTML via Canvas `module_items` + page/assignment APIs). After `nest build`, the builder loads `ACCESSIBILITY_FIXABILITY_MAP` from **`dist/canvas/canvas.service.js`**. Phase 2 runner: scanner + fix assertions; manifest rows scale with `fixtures.json` (page clones: ann/disc/quiz/mod per page violation; mod per assignment violation). **`correct_link_href`** drives **`link_broken`** suggested fix (teacher URL) in fix-mode. **`QA_ASSERT_ZERO_AI_HEURISTIC`** (default on; set `=0` to disable) asserts **zero** Anthropic tokens on preview for manifest rows with **`uses_ai: false`**. **`QA_BASELINE_REPORT`** strict regression compare remains available. **Bulk Editor UI:** Accessibility tab includes **quizzes** and **modules** in resource-type checkboxes (default all types selected) and an **AG Grid** view for loaded **`report-qa-*.json`** files. **CI:** `.github/workflows/ci.yml` runs **`npm ci`**, **`build`**, **`test`** (no live Canvas QA). **Re-verify** strict scan + fix counts locally after pulling: fresh **`qa:accessibility:build:force`** then **`qa:accessibility:run`** / **`qa:accessibility:run:fix`**; commit a new golden report when stable.

**Data flow:** Builder → Canvas + **`test/fixtures/accessibility-qa/manifest.json`** → Runner calls Nest **`/canvas/courses/:id/accessibility/scan`** with QA headers → Runner writes **`report-<run_id>.json`** (optional **`baseline_compare`** when **`QA_BASELINE_REPORT`** is set).

The bulk editor UI uses **AG Grid** for course tooling and for **loading `report-qa-*.json`** in the Accessibility tab (fixture-level rows).
...
5. Add Announcements, Discussions, Syllabus (composite strategy). — **done**
6. Add Quizzes + Module items: **capability probe** first (§3.3.1); manifest `skipped_reason` when API cannot preserve HTML; fallback coverage on Pages. — **done** (classic quiz `description` + first-quiz read-back probe; module Page/Assignment items; New Quizzes out of scope)
7. Add file upload pipeline + linked HTML for file-based rules. — **partial** (Canvas two-step file upload in builder for `use_course_pdf_placeholder` rows; static `https://example.com/…` links cover other doc rules in fixtures)
...
2. ~~Scanner assertion engine; respect **strict** vs **best_effort** gates.~~ — **done** (strict OSS **68/68** full manifest, `report-qa-1774715833910.json`; earlier **23/23**, `report-qa-1774710091861.json`).
3. ~~Fix-preview + fix-apply loop + re-scan for `auto` rules using **sequential_reapply** or **single_rule_per_resource** (§4.2.1).~~ — **done** for harness + non-AI `auto` rows (`npm run qa:accessibility:run:fix`, `report-qa-1774717260033.json`, **`fix_fail=0`**); **not** full matrix / every rule family; optional **`run:fix:ai`** for `uses_ai` auto.
4. ~~Dual-option fix validation for `aria_hidden_focusable` and `table_layout_heuristic` (§4.2.2).~~ — **done** (included in same **`report-qa-1774717260033.json`** fix-mode run).
5. Extend to all content types in manifest; quiz capability probe skips documented in report. — **done** for classic quizzes + module items in scan/builder/manifest.
6. HTML well-formedness + reversibility Level A/B where supported. — **not done** (parser-based fragment validation not wired; fix path relies on re-scan + token asserts).
7. AI token validation — assert zero AI tokens for heuristic rules; assert correct model for remaining AI rules. — **partial** (zero-token assert for `uses_ai: false` on fix-preview; model-level assert not implemented).
8. Report JSON (tiered rollups + AI cost section) + AG Grid integration. — **done** for JSON + **AG Grid** load of QA report files in Accessibility tab (`ai_heuristic_assert_failures`, `fix_meter_aggregate` in report).
9. Regression baseline compare (strict-only optional for CI). — **partial** (`QA_BASELINE_REPORT` / `QA_BASELINE_PATH`; CI does not run Canvas QA).
10. CI documentation and staging-only wiring. — **partial** (Jest + build in GitHub Actions; full QA remains local OSS per prerequisites).
...
**Where we are (Mar 2026):** **Scanner** supports **`pages`**, **`assignments`**, **`announcements`**, **`discussions`**, **`syllabus`**, **`quizzes`**, **`modules`**. **Builder** mirrors page violations into **announcement**, **discussion**, **quiz**, and **module Page** rows; mirrors assignment violations into **module Assignment** rows; **merged syllabus** composite; **`correct_link_href`** on **`link_broken`** fixtures; **SproutVideo** + **YouTube**, **Vimeo**, **Google Docs** iframe rows; **doc** / **video** / **audio** scanner fixtures; optional **course PDF upload** row. **Strict counts** are **not** the historical 68 — re-run builder + runner and refresh baseline reports after fixture expansion. **Fix-mode** includes **`link_broken`** teacher-href simulation and **`QA_ASSERT_ZERO_AI_HEURISTIC`** on heuristic rules. **UI:** Accessibility tab defaults all resource types (including quizzes/modules) and can load **`report-qa-*.json`** into **AG Grid**. **CI:** build + unit tests only.
...
- [x] Builder (`scripts/accessibility-qa-builder.js`) — course create/reuse, **`--force-rebuild`**, **pages + assignments + announcements + discussions + quizzes + module items**, **merged syllabus**, optional **PDF upload** (`use_course_pdf_placeholder`), **`canvas_capability`**, manifest emit, **registry sync from `dist`**.
- [x] Runner (`scripts/accessibility-qa-runner.js`) — manifest load + schema validation, **strict/best-effort** scanner assertions, JSON report, **`QA_FIX_AUTO`**, dual-option suggested fixes, **`link_broken`** via **`correct_link_href`**, **`QA_LINK_SCAN_RETRIES`**, **`QA_BASELINE_REPORT`**, **`QA_ASSERT_ZERO_AI_HEURISTIC`** (zero tokens when **`uses_ai: false`**), **`ai_heuristic_assert_failures`** in report.
...
- [x] **Iframe fixtures (SproutVideo-first):** **SproutVideo** on **pages + assignments** — **done**. **YouTube**, **Vimeo**, **Google Docs** `iframe_missing_title` rows on **pages** — **done** (§0.8, §3.7).
