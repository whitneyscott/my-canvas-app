...
- [x] Runner (`scripts/accessibility-qa-runner.js`) — **done for v1:** manifest load + schema validation, **strict/best-effort** scanner assertions, JSON report, optional **`QA_FIX_AUTO`** path. **Verified:** strict **pass 19/0** on Canvas OSS with iframe fixtures (`report-qa-1774709610791.json`, Mar 2026). **Still missing:** dual-option *fix* tests, AI token assertions, suggested-flow simulation, regression baseline diff.
